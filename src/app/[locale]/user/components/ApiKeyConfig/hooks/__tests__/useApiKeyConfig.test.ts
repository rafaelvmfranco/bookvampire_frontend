...
vi.mock("~/libs/api/auth/useAuthApi", () => ({
  useAuthApi: () => useAuthApiMock(),
}));
vi.mock("~/services/analytics/june/useEventTracking", () => ({
  useEventTracking: () => useEventTrackingMock(),
}));
vi.mock("~/libs/context/SupabaseProvider", () => ({
  useSupabase: () => mockUseSupabase(),
}));

vi.mock("~/libs/hooks", async () => {
  const actual =
    await vi.importActual<typeof import("~/core/libs/hooks")>("~/libs/hooks");
